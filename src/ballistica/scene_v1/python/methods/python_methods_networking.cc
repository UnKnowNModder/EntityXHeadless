...
static auto PyGetClientIP(PyObject* self, PyObject* args, PyObject* keywds) -> PyObject* {
  BA_PYTHON_TRY;
  int client_id;
  ...
  auto* appmode = classic::ClassicAppMode::GetActiveOrThrow();
  std::string ip = appmode->connections()->GetClientIP(client_id);
  return PyUnicode_FromString(ip.c_str());
  BA_PYTHON_CATCH;
}

static PyMethodDef PyGetClientIPDef = {
    "get_client_ip",              // name
    (PyCFunction)PyGetClientIP,   // method
    METH_VARARGS | METH_KEYWORDS, // flags

    "get_client_ip(client_id: int) -> str\n"
    "\n"
    "Return the IPv4 or IPv6 address for a connected client.\n"
    "Returns an empty string if client_id is invalid.\n"};
